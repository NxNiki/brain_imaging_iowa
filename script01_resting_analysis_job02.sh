...
TASKDIR=${ETKINLAB_DIR}/3_Data_Working/CausalConnectome/rest/
...
# just use one mask to get the 4d image and extract roi signals in the next step:
masks=("FIRST_L_amyg_small")


cd $TASKDIR

#for i in `ls -r ${PWD}/CausCon_NTHC10[4-6]*.nii.gz`  ; do
for i in `ls -r ${PWD}/CausCon_TEHC20[4-6]*.nii.gz`  ; do
    ...
    if [ ! -d ${i}.${TARGETDIR} ] ; then
        ...
        if [ `imtest $T1` = 1 ] && [ -d ${T1}.struct_only ]; then
            for m in "${masks[@]}"; do
                #qsub analysis_pipeline_Jing.sh -resting -no_resting_gm_mask -func_data $i -t1 ${T1} -tr 2 -deleteVolumes 6 -reg_info ${T1}.struct_only -fc_rois_mni ${MASKDIR}/${m}.nii.gz -atlas_conn_opts --useAllLabels -motion -output_extension ${TARGETDIR}
