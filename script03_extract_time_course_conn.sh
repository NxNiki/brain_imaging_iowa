...
output_dir="/Shared/jianglab/3_Data_Working/CausalConnectome/rest_output_conn"
...
for img_4d in `ls $img_dir/conn_*HC*/results/preprocessing/3comp*.nii`;
do
  echo $img_4d
  #atlases="Atlases/"
  #subject_id=$(echo $img_4d | awk -F "/" "{print $1}" | sed 's/CausCon_//' | sed 's/_rest.rest//')
